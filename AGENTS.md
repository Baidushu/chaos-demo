...
- **常用环境变量**：`REDIS_HOST/PORT`、`ENABLE_RESILIENCE`、`RATE_LIMIT_*`、`BREAKER_*`、`BUSINESS_TIMEOUT_MS`、`INVENTORY_BUSY_PROB`、`ORDER_TTL_SEC`、`TRAFFIC_RECORD_ENABLED/FILE`、`LOG_FORMAT`、`ENABLE_FAULT_INJECTION`、`CHAOS_ADMIN_TOKEN`（`/fault/*`、`/chaos/*` 写操作令牌；留空=放行但启动告警）、`TRUST_PROXY_HEADERS` + `XFF_TRUSTED_HOPS`（限流维度是否采信 `X-Forwarded-For`，默认不采信）、`CHAOS_SERVICE_URL`、`CHAOS_LOG_FILE`、`INCIDENT_LLM_ENABLED`、`AGENT_MODE`、`AGENT_EVAL_SKIP_JUDGE`、`TOOLS_BASE_URL`、`PLATFORM_SECURITY_POLICY`（安全策略文件路径覆盖，默认 `config/security_policy.yaml`，策略文件即代码：版本化角色工具权限，schema 严格校验，未知角色 fail-closed；显式传 config 时不做文件覆盖）。
...
- **管理接口鉴权与代理头信任（2026-09 加固）**：`/fault/*`、`/chaos/*` 的**写操作**由 `app/api/admin_auth.py` 的前缀+方法级 `before_request` 守卫保护（fail-closed：将来新增写接口默认已被保护）。令牌来自 `CHAOS_ADMIN_TOKEN`（**仓库内不内置默认口令**）：已配置 → 必须带 `X-Chaos-Admin-Token`（`hmac.compare_digest` 常量时间比较，401 不回显期望值）；未配置 → 放行但**启动打 WARNING**。只读接口（`GET /fault/status`、`GET /chaos/experiments`）保持公开。限流维度侧：`chaos_service/rate_limiter.py::client_ip_from_request` **默认不采信 `X-Forwarded-For`**（可伪造 → 换 IP 绕过限流），需显式 `TRUST_PROXY_HEADERS=true` 才按 `XFF_TRUSTED_HOPS`（默认 1）**从右往左**取可信跳并丢弃最左可伪造值，跳数不足退回 `remote_addr`；非 `client_ip` 维度不再读该头。回归测试：`tests/unit/test_admin_auth.py`、`tests/unit/test_rate_limiter_forwarded.py`；`fault_demo.py` 会自动带上令牌头并在 401 时提示。
