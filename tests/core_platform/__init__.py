...
class TestModelConfig:
    ...
    def test_custom(self):
        c = ModelConfig(provider="ollama", model="llama3", temperature=0.7, max_tokens=1024, timeout_seconds=15.0)
...
class TestEvaluationConfig:
    def test_defaults(self):
        ...
        assert c.hallucination_rate_max == 0.1

    def test_custom(self):
        c = EvaluationConfig(tool_selection_accuracy_min=0.9, retry_rate_max=0.1)
        assert c.tool_selection_accuracy_min == 0.9
        assert c.retry_rate_max == 0.1
...
class TestPlatformConfig:
    def test_defaults(self):
        ...
        assert c.evaluation_enabled is False  # disabled for single-request mode
        assert c.quality_gate_enabled is False
    ...
    def test_from_dict(self):
        ...
        assert c.model.provider == "ollama"
        assert c.timeout_seconds == 120.0
        assert c.evaluation_enabled is True
        assert c.quality_gate_enabled is True

    def test_from_dict_missing_keys(self):
        c = PlatformConfig.from_dict({})
        assert c.mode == "rule"
        assert c.model.provider == "mock"

    def test_default_classmethod(self):
        c = PlatformConfig.default()
        assert isinstance(c.security, SecurityPolicy)

    def test_security_is_preserved_across_serialization(self):
        policy = SecurityPolicy(max_input_length=1234)
        c = PlatformConfig(security=policy)
        d = c.to_dict()
        restored = PlatformConfig.from_dict(d)
        assert restored.security.max_input_length == 1234
